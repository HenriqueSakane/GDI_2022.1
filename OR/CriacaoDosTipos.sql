...
CREATE OR REPLACE TYPE tp_funcionario UNDER tp_pessoa (
	cargo REF tp_cargo,
	OVERRIDING MEMBER FUNCTION nomePessoa(P tp_pessoa) RETURN VARCHAR2
) NOT FINAL NOT INSTANTIABLE;
/
--Overriding Member Function de Pessoa
CREATE OR REPLACE TYPE BODY tp_funcionario 
IS 
OVERRIDING MEMBER FUNCTION nomePessoa(f tp_funcionario) RETURN VARCHAR2 AS nomeFuncionario 
IS
    BEGIN
        RETURN f.nome;
    END;
